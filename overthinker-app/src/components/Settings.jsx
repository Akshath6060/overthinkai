...
export default function Settings(v) {
  const selectedTheme = v.account?.settings?.theme || 'cream';
  const themeValues = ['cream', 'light', 'dark'];
  const themes = themeLabels.map((t, i) => ({ label: t, value: themeValues[i], bg: selectedTheme === themeValues[i] ? '#FFD84D' : '#FFF', sh: selectedTheme === themeValues[i] ? SHADOW : 'none' }));
  return (
    <div style={css('max-width:880px;margin:0 auto;display:flex;flex-direction:column;gap:16px')}>
      ...
      <div style={css('background:#FFF;border:3px solid #2B2336;border-radius:18px;padding:18px;box-shadow:5px 5px 0 #2B2336')}>
        ...
        <div style={css('display:flex;gap:9px;flex-wrap:wrap')}>
          {themes.map((t, i) => (
            <button key={t.value} type="button" aria-pressed={selectedTheme === t.value} disabled={v.settingsSaving} onClick={() => v.persistSetting('theme', t.value)} style={css(`padding:10px 16px;border-radius:12px;border:2.5px solid #2B2336;background:${t.bg};color:#1A1720;font-size:12.5px;font-weight:700;cursor:pointer;box-shadow:${t.sh}`)}>{t.label}</button>
          ))}
        </div>
      </div>
      ...
      <div style={css('background:#FFF;border:3px solid #2B2336;border-radius:18px;padding:18px;box-shadow:5px 5px 0 #2B2336')}>
        ...
        <div style={css('display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px')}>
          {v.levels.map(l => (
            <button key={l.name} type="button" aria-pressed={l.on} disabled={v.settingsSaving} onClick={l.pick} style={css(`text-align:left;padding:12px 14px;border-radius:13px;border:3px solid #2B2336;background:${l.bg};cursor:pointer;box-shadow:${l.sh}`)}>
      ...
      <div style={css('background:#FF4FA3;border:3px solid #2B2336;border-radius:18px;padding:18px;box-shadow:5px 5px 0 #2B2336')}>
        ...
        <div style={css('display:flex;gap:5px;padding:5px;border:3px solid #2B2336;background:#FFF;border-radius:14px;width:fit-content;max-width:100%;flex-wrap:wrap')}>
          {v.humorOpts.map(h => (
            <button key={h.label} type="button" aria-pressed={h.on} disabled={v.settingsSaving} onClick={h.pick} style={css(`padding:8px 17px;border-radius:9px;border:2px solid ${h.bd};background:${h.bg};color:#1A1720;font-family:'Space Grotesk',sans-serif;font-size:13px;font-weight:700;cursor:pointer`)}>{h.label}</button>
          ))}
        </div>
      ...
      <div style={css('background:#FFF;border:3px solid #2B2336;border-radius:18px;overflow:hidden;box-shadow:5px 5px 0 #2B2336')}>
        {v.toggleRows.map(t => (
          <div key={t.k} style={css('display:flex;align-items:center;gap:14px;padding:15px 18px;border-bottom:2px solid #EFE6D4')}>
            ...
            <button type="button" role="switch" aria-checked={t.on} aria-label={t.label} disabled={v.settingsSaving} onClick={t.toggle} style={css(`width:46px;height:26px;flex:none;border-radius:999px;border:2.5px solid #2B2336;background:${t.trackBg};cursor:pointer;padding:0;display:flex;align-items:center;justify-content:${t.knobPos};transition:all .18s`)}>
      ...
      <div style={css('background:#FFD84D;border:3px solid #2B2336;border-radius:18px;padding:18px;box-shadow:5px 5px 0 #2B2336;display:flex;flex-wrap:wrap;gap:14px;align-items:center')}>
        ...
        <div style={css('display:flex;gap:9px;flex-wrap:wrap')}>
          <button type="button" disabled={v.settingsSaving} onClick={v.exportData} style={css('padding:10px 15px;border-radius:12px;border:2.5px solid #2B2336;background:#FFF;color:#1A1720;font-size:12.5px;font-weight:700;cursor:pointer;box-shadow:3px 3px 0 #2B2336')}>Export JSON</button>
          <button type="button" disabled={v.settingsSaving} onClick={() => { if (window.confirm('Delete all saved decision history? This cannot be undone.')) v.deleteHistory(); }} style={css('padding:10px 15px;border-radius:12px;border:2.5px solid #2B2336;background:#FF4D4D;color:#FFF;font-size:12.5px;font-weight:700;cursor:pointer;box-shadow:3px 3px 0 #2B2336')}>Delete All Evidence</button>
        </div>
      </div>
      {v.settingsError && <div role="alert" style={css('padding:12px 14px;border:3px solid #2B2336;border-radius:12px;background:#FFB3B3;font-weight:700')}>{v.settingsError}</div>}
      {v.settingsSaving && <div role="status" aria-live="polite" style={css('font-size:12px;font-weight:700;color:#6F687A')}>Saving the unnecessarily precise configuration…</div>}
    </div>
  );
}
