...
Required production variables: `APP_ENV`, `MONGODB_URI`, `MONGODB_DATABASE`, `GEMINI_API_KEY`, `GEMINI_MODEL`, `GEMINI_JUDGE_MODEL`, `GEMINI_FALLBACK_MODELS`, `GEMINI_MAX_CONCURRENCY`, `SESSION_SECRET`, `SESSION_EXPIRE_DAYS`, `FRONTEND_URL`, `ALLOWED_ORIGINS`, `DEFAULT_CREDIT_ALLOWANCE`, `AI_PROVIDER`, `LOG_LEVEL`, `COOKIE_SECURE`, `COOKIE_SAMESITE`, and `DOCS_ENABLED`. The `OPENAI_*` variables remain available when `AI_PROVIDER=openai`. Gemini requests rotate through the ordered `GEMINI_FALLBACK_MODELS` pool after retryable capacity, quota, or retired-model responses. Requests share a process-wide concurrency gate so simultaneous analysts cannot amplify a provider quota response into a retry storm.
